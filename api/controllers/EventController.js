...
module.exports = {
  ...
  update: function (req, res, next) {
    ...
    if (status && title && req.isSocket) {
      Event.update(id, {status: status, title: title ,isComplete:isComplete}).exec(function update(err, updated) {
        Event.publishUpdate(updated[0].id, { status: updated[0].status, title: updated[0].title });
      })
    }
    else {
      if (status && req.isSocket) {
        Event.update(id, {status: status,isComplete:isComplete }).exec(function update(err, updated) {
            Event.publishUpdate(updated[0].id, { status: updated[0].status });
        })
      }
    }
  },
